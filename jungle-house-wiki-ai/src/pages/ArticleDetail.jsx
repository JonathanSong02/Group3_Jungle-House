...
export default function ArticleDetail() {
    ...
    fetch(`http://127.0.0.1:5000/articles`)
      ...
      .then((data) => {
        const found = data.find(
          (item) => item.article_id === parseInt(id)
        );
        setArticle(found);
      })
  ...
  // 🔥 GET LINKS
  useEffect(() => {
    fetch(`http://127.0.0.1:5000/article-links/${id}`)
      .then((res) => res.json())
...
<pre style={{ whiteSpace: "pre-wrap", fontFamily: "inherit" }}>
  {article.content.split("\n").map((line, index) => {
    ...
    // ✅ FIXED IMAGE DETECTION (IMPORTANT)
    ...
    // ✅ TEXT + CUSTOM CLICKABLE LINKS
    const urlRegex = /(https?:\/\/[^\s]+)/g;

// First apply custom links
let elements = renderLineWithLinks(line);

// Then apply URL links
let finalElements = [];
